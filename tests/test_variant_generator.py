...
class TestVariantSpecs:
    ...
    def test_factories_are_callable(self):
        ...
        # Replace infrastructure-heavy production classes while retaining the
        # real dimensions, constraints, and derived parameters. This keeps the
        # unit test independent of collection order and kubeconfig state.
        specs = [
            VariantSpec(
                problem_class=FakeProblem,
                ...
                derived_params=spec.derived_params,
            )
            for spec in get_all_variant_specs()
        ]
        all_variants = generate_all_variants(specs)
...
class TestGenerateVariantStreamByClass:
    ...
    def test_all_classes_in_every_full_round(self):
        ...
        num_classes = 3
        num_rounds = 4
        stream = generate_variant_stream_by_class(ids, count=num_classes * num_rounds, seed=7)
        for r in range(num_rounds):
            start = r * num_classes
            round_items = stream[start : start + num_classes]
            classes = {v.split("__v_")[0] for v in round_items}
            assert classes == {"big", "small", "medium"}, f"Round {r} classes: {classes}"
...
class TestGenerateVariantStreamGrouped:
    ...
    def test_drains_one_class_before_next(self):
        ...
        block_starts = [i for i in range(len(classes)) if i == 0 or classes[i] != classes[i - 1]]
    ...
    def test_max_per_class_caps_block_size(self):
        ...
        stream = generate_variant_stream_grouped(ids, count=4, seed=42, max_per_class=2)
        ...
        classes = self._classes_of(stream)
        boundary = [i for i in range(len(classes)) if i == 0 or classes[i] != classes[i - 1]]
    ...
    def test_max_per_class_larger_than_group(self):
        ...
        stream = generate_variant_stream_grouped(ids, count=7, seed=42, max_per_class=5)
    ...
    def test_offset_with_max_per_class(self):
        ...
        full = generate_variant_stream_grouped(ids, count=12, offset=0, seed=42, max_per_class=3)
        tail = generate_variant_stream_grouped(ids, count=6, offset=6, seed=42, max_per_class=3)
        assert full[6:] == tail
    ...
    def test_invalid_max_per_class_raises(self):
        with pytest.raises(ValueError, match="max_per_class must be > 0"):
            generate_variant_stream_grouped(["a__v_0"], count=1, max_per_class=0)
...
class TestFilterVariantIdsBySpec:
    ...
    def test_works_with_real_specs(self):
        ...
        assert len(result) > 0
        assert all(vid.startswith("readiness_probe_misconfiguration__v_") for vid in result)
...
class TestAdaptiveScheduler:
    ...
    def test_seq_idx_is_monotonic(self):
        sched = AdaptiveScheduler(self.POOL, consec_solves_to_stop=10, max_per_class=3, seed=7)
        seqs = []
        for _ in range(5):
