...
def ensure_kind_images(cluster_name: str, images: list[str]) -> dict[str, dict[str, str]]:
    ...
    for node in _kind_nodes(cluster_name):
        ...
        for image, digest in expected.items():
            ...
            if observed != digest:
                raise RuntimeError(
                    f"{node} image {image} digest mismatch: expected {digest}, observed {observed}"
                )
...
def platform_image_archive(image: str, platform_name: str) -> tuple[str, str]:
    handle = tempfile.NamedTemporaryFile(prefix="sregym-image-", suffix=".tar", delete=False)
    handle.close()
    archive = handle.name
...
def preflight_cluster_requirements(cluster_name: str) -> None:
    ...
    if _truthy_env("SREGYM_KIND_REQUIRE_NETWORK_POLICY"):
        ...
        if canary_image:
            verify_network_policy_enforcement(cluster_name, canary_image)
...
def install_calico(cluster_name: str, kubeconfig_path: str) -> None:
    manifest = tempfile.NamedTemporaryFile(prefix="calico-", suffix=".yaml", delete=False)
    manifest.close()
    try:
        _run(["curl", "-fsSL", _CALICO_URL, "-o", manifest.name])
        manifest_bytes = Path(manifest.name).read_bytes()
        ...
        _run(["kubectl", "--kubeconfig", kubeconfig_path, "create", "-f", manifest.name])
    finally:
        try:
...
def verify_network_policy_enforcement(cluster_name: str, canary_image: str) -> None:
    if not canary_image:
...
def create_kind_cluster(cluster_name: str, kubeconfig_path: str) -> tuple[str, str]:
    ...
    try:
        ...
        preflight_cluster_requirements(cluster_name)
    finally:
        try:
