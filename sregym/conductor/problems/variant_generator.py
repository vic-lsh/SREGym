...
def generate_variants(spec: VariantSpec) -> dict[str, Callable]:
    ...
    for combo in product(*dim_values):
        params = dict(zip(dim_names, combo, strict=True))
    ...
    for spec in specs:
        ...
        for vid, factory in generated.items():
            if vid in all_variants:
                raise ValueError(f"Variant ID collision: '{vid}' generated by multiple specs")
...
def filter_variant_ids_by_spec(
    ...
    if unknown:
        raise ValueError(f"Unknown variant spec name(s): {unknown}. Valid names: {sorted(known_base_names)}")
...
class AdaptiveScheduler:
    ...
    def __init__(
        ...
        self._recent: dict[str, deque[bool]] = {name: deque(maxlen=consec_solves_to_stop) for name in self._class_order}
    ...
    def set_next_seq_idx(self, value: int) -> None:
        if value < self._next_seq_idx:
            raise ValueError(f"set_next_seq_idx must not go backwards (have {self._next_seq_idx}, got {value})")
